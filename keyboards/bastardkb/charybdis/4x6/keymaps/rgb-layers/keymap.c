...
report_mouse_t pointing_device_task_user(report_mouse_t mouse_report) {
    if (abs(mouse_report.x) > CHARYBDIS_AUTO_POINTER_LAYER_TRIGGER_THRESHOLD || abs(mouse_report.y) > CHARYBDIS_AUTO_POINTER_LAYER_TRIGGER_THRESHOLD) {
        if (auto_pointer_layer_timer == 0) {
            layer_on(LAYER_POINTER); // Just change the layer
        }
...
void matrix_scan_user(void) {
    if (auto_pointer_layer_timer != 0 && TIMER_DIFF_16(timer_read(), auto_pointer_layer_timer) >= CHARYBDIS_AUTO_POINTER_LAYER_TRIGGER_TIMEOUT_MS) {
        ...
        layer_off(LAYER_POINTER); // Just change the layer
    }
}
#    endif // CHARYBDIS_AUTO_POINTER_LAYER_TRIGGER_ENABLE
...
layer_state_t layer_state_set_user(layer_state_t state) {
    // This line is from your original code, it keeps auto-sniping working.
    charybdis_set_pointer_sniping_enabled(layer_state_cmp(state, CHARYBDIS_AUTO_SNIPING_ON_LAYER));

    // This is the new part that changes color based on the active layer.
    switch (get_highest_layer(state)) {
        case LAYER_BASE:
            // Set base layer to a warm white
            rgb_matrix_sethsv_noeeprom(HSV_WHITE);
            break;
        case LAYER_LOWER:
            // Set lower layer to blue
            rgb_matrix_sethsv_noeeprom(HSV_BLUE);
            break;
        case LAYER_RAISE:
            // Set raise layer to red
            rgb_matrix_sethsv_noeeprom(HSV_RED);
            break;
        case LAYER_POINTER:
            // Set pointer layer to green
            rgb_matrix_sethsv_noeeprom(HSV_GREEN);
            break;
        default:
            // Optional: for any other layers, do nothing or set a default.
            break;
    }
    return state;
}
...
void keyboard_post_init_user(void) {
    // Set the RGB mode to solid color, disabling animations
    rgb_matrix_mode_noeeprom(RGB_MATRIX_SOLID_COLOR);
    // Set the initial color to white for the base layer
    rgb_matrix_sethsv_noeeprom(HSV_WHITE);
}
